...
def perpareToJSON(dic1, dic2, blackListPaht):
	string = '['
	...
	for key in dic1:
		if key in dic2:
			string += "{ 'label':'" + str(key).replace("'",'"') + "', 'alert':'" + str(key in lines) + "', 'value1':'" + str(dic1[key]).replace("'",'"') + "', 'value2':'" + str(dic2[key]).replace("'",'"') + "'},"
			#string += "{ 'label':'" + str(key).replace("'",'"') + "', 'alert':'" + str(alert) + "', 'value1':'" + str(dic1[key]).replace("'",'"') + "', 'value2':'" + str(dic2[key]).replace("'",'"') + "'},"
		else:
			string += "{ 'label':'" + str(key).replace("'",'"') + "', 'alert':'" + str(key in lines) + "', 'value1':'" + str(dic1[key]).replace("'",'"') + "', 'value2':' ABSENT'},"
	for key in dic2:
		if key not in dic1:
			string += "{ 'label':'" + str(key).replace("'",'"') + "', 'alert':'" + str(key in lines) + "', 'value1':'ABSENT', 'value2':'" + str(dic2[key]).replace("'",'"') + "'},"
	return string+"]"
...
print "var data = " + perpareToJSON(qResult, gResult, blackList) + ";"
